...
const RAW_DATA = d3.csv('data/teslaData.csv', d => {
    console.log("csvData", d)
    return {
        model: d['Model'],
...
RAW_DATA.then(data => {
    ...
    const model = (model) => model.toUpperCase();
    // function model(model) { return model.toUpperCase(); }
    const price = (price) => d3.format('$,.2f')(price);
...
//     }
    ...
    const displayInfo = (car, colorIndex) => {
        ...
        d3.selectAll('#mInfo p')
            .style('color', COLOR((colorIndex + 1) / DATA.length))
    }
